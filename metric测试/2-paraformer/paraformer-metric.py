...
def main():
  ...
  json_report_path = "./paraformer_sichuan_eval_report_1000samples.json"
  with open(json_report_path, "w", encoding="utf-8") as f:
    json.dump(report, f, ensure_ascii=False, indent=4)

  # 保存简易文本报告（便于阅读和对比）
  txt_report_path = "./paraformer_sichuan_eval_report_1000samples.txt"
  with open(txt_report_path, "w", encoding="utf-8") as f:
    f.write("=" * 60 + "\n")
    f.write(f"Paraformer-large-vad-punc 四川方言评测报告（前{MAX_SAMPLES}条）\n")
    f.write("=" * 60 + "\n")
  ...
  # 5. 打印最终结果
  print("\n" + "=" * 60)
  print(f"✅ Paraformer-large-vad-punc 评测完成（前{MAX_SAMPLES}条）！")
  print(f"📊 核心指标：")
