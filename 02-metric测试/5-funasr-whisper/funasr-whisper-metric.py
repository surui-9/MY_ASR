...
def resample_audio(input_path, output_path="./temp_16k_whisper.wav"):
  """将任意音频转为 Whisper 要求的 16kHz 单声道 WAV"""
  y, sr = librosa.load(input_path, sr=None)
  # 转单声道
  if len(y.shape) > 1:
    y = librosa.to_mono(y)
  # 重采样到 16kHz（Whisper最优输入格式）
  if sr != 16000:
...
def whisper_turbo_inference(model, audio_path: str) -> str:
  """Whisper-large-v3-turbo 推理封装，完全适配你的配置"""
  try:
    # 第一步：统一音频格式（提升识别稳定性）
    audio_16k = resample_audio(audio_path)

    # 第二步：Whisper推理（加载你的解码参数）
    result = model.generate(
        input=audio_16k,
        cache={},
        batch_size=1,
        decoding_options=DECODING_OPTIONS,  # 传入你的解码配置
...
def main():
  ...
  model = AutoModel(
      ...
      disable_update=True,
      device=DEVICE  # 显式指定设备，和你的配置一致
  )
  print(f"✅ Whisper-large-v3-turbo模型加载完成！")
  ...
  pbar = tqdm(valid_samples, desc="Whisper-large-v3-turbo评测中")
  for sample in pbar:
    try:
      ...
      # 模型推理
      pred_text = whisper_turbo_inference(model, audio_path)

      # 计算Metric（和所有模型用同一套逻辑）
  ...
  json_report_path = "./whisper_large_v3_turbo_sichuan_eval_report_1000samples.json"
